...
import networking.request.RequestMove;
...
public class GameClient extends Thread {
    ...
    public GameClient(Socket clientSocket, GameServer server) throws IOException {
        ...
        //activePlayers.put(playerId, new Player(playerId));
            ...
            try {
                ...
                if (requestLength > 0) {
                    ...
                    if (request != null) {
                        ...
                        if(requestCode == Constants.REQ_HEARTBEAT)
                        {
                        ...
                        else if (requestCode == Constants.CMSG_REGISTER){
                        ...
                        else if (requestCode == Constants.CMSG_AUTH){
                        	 for (GameResponse response : request.getResponses()) {
                                 // Transform the response into bytes and pass it into the output stream
                                 System.out.println(response);
                                 
                                 //playerId = ((RequestLogin)request).getUsername();
                                 //request.setPlayerId(playerId);
                                 //this.activePlayers = activePlayers;
                                 //this.server.addActivePlayer.put(playerId, new Player(playerId));
                                 outputStream.write(response.constructResponseInBytes());
                              }
						} else if (requestCode == Constants.CMSG_CHAT_ALL) {
							for (GameResponse response : request.getResponses()) {
								// Transform the response into bytes and pass it
								// into the output stream
								updates.add(response);

								server.addResponseForAllOnlinePlayers(
										this.getId(), response);
                        }

						} else if (requestCode == Constants.CMSG_CHAT_ONE) {
							for (GameResponse response : request.getResponses()) {
								// Transform the response into bytes and pass it
								// into the output stream
								updates.add(response);
								ResponseChatOne temp = ((ResponseChatOne) response);
								server.addResponseForOneOnlinePlayer(
										temp.getReceiverId(), response,
										(String) temp.getSenderId());
                            ...
                            for (GameResponse response : request.getResponses()) {
                                // Transform the response into bytes and pass it into the output stream
                                 ...
                                 server.addResponseForAllOnlinePlayers(this.getId(), response);
                            }
                        }
                    }
                } else {
                    ...
                    if ((System.currentTimeMillis() - lastActivity) / 1000 >= Constants.TIMEOUT_SECONDS) {
                        isPlaying = false;
                        this.server.removeActivePlayer(this.playerId);
                    }
                }
            } catch (Exception e) {
