...
class Chat:
    def __init__(self, connection, world):
        ...
        self.b = DirectEntry(parent=self.chatFrame, text="", scale=.05,
                             ...
                             initialText="<Click to Chat>", numLines=1, focus=1, focusInCommand=self.clearText,
    ...
    def show(self, other_id):
        ...
        self.updateChat(other_id)

    def updateChat(self, other_id):  # callback function to set text
        self.chatObject.destroy()

        while self.chat_memory[other_id].__len__() >= 15:
            del self.chat_memory[other_id][0]
        self.chat_text = ""
        i = self.chat_memory[other_id].__len__()
        for line in self.chat_memory[other_id]:
            self.i = i - 1
    ...
    def sendChat(self, text_entered):
        ...
        if self.current_chat_window == 0:
            ...
            self.cManager.sendRequest(106, [self.world.mainCharRef.playerId, self.current_chat_window, text_entered])
            ...
            # else:
            # broadcast to user
            #     self.cManager.sendRequest(106, textEntered)
